...
class VisaService {
    doMutualAuthRequest(path, requestBody, methodType, headers, callback) {
        ...
        node_fetch_1.default(config.visaUrl + path, { method: methodType, headers: headers, body: requestBody, key: KEY, cert: CERT })
            .then(r => r.json())
            .then((response) => {
    ...
    doXPayRequest(baseUri, resourcePath, queryParams, requestBody, methodType, headers, callback) {
        ...
        node_fetch_1.default(config.visaUrl + baseUri + resourcePath + '?' + queryParams, { method: methodType, headers: headers, body: requestBody })
            .then(r => r.json())
            .then((response) => {
            callback(null, response);
        }).catch(e => callback(e));
    ...
    getXPayToken(resourcePath, queryParams, postBody) {
        ...
        var hashString = crypto_1.createHmac('SHA256', sharedSecret).update(preHashString).digest('hex');
        var preHashString2 = resourcePath + queryParams + postBody;
        var hashString2 = crypto_1.createHmac('SHA256', sharedSecret).update(preHashString2).digest('hex');
        var xPayToken = 'xv2:' + timestamp + ':' + hashString;
        return xPayToken;
    }
}
exports.VisaService = VisaService;
exports.default = VisaService;
